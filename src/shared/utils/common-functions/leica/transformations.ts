...
export const getAutomaticIndex = function (word: string): string | void {
...
// GSI8
export const getAngle = (survey: string, format_length: number): number | void => {
...
export const getDistanceInMt_Ft = function (word: string, format_length: number): number {
  const unit = getUnitCode(word);
  const numbersArray = word
    .trim()
    .split('')
    .slice(-format_length - 1);
  let divisor = 1000;
