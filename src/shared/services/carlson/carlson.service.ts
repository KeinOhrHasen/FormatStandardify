  ...
  private splitOnStations(stringToParse: string): any {
    ...
    splittedOnRows.forEach((row, index) => {
      ...
      if (
        row.startsWith('BP') ||
        (row.startsWith('LS') &&
          !splittedOnRows[index - 1].startsWith('BP') &&
          !splittedOnRows[index - 2].startsWith('BP'))
      ) {
        if (station.length !== 0) {
    ...
    const newStations = (stationsToParse.stations as any[]).map((station: any[]) => {
      ...
      station.forEach((element, index: number) => {
        ...
        if (
          index === 0 ||
          element.includes('LS') ||
          element.includes('Entered Rover') ||
          element.includes('Antenna Type')
        ) {
